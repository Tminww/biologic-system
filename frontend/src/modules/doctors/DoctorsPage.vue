...
import type { ReadDoctorDTO } from './doctors.api'
import { createDoctor, deleteDoctor, listDoctors, updateDoctor } from './doctors.api'
...
const columns = computed<TableColumn[]>(() => [
  ...
  },
  {
    ...
    sortable: true,
    filter: { type: 'text', placeholder: t('filter.default') },
    body: (row: ReadDoctorDTO) => [row.last_name, row.patronymic].filter(Boolean).join(' ') || '-'
  },
  {
    field: 'updated_at',
    ...
    body: (row: ReadDoctorDTO) => formatDateTime(row.updated_at)
  }
])
...
const table = useServerTable<ReadDoctorDTO>(listDoctors, {
  presetKey: 'doctors',
...
const dialog = useCrudDialog<ReadDoctorDTO>('doctors')
...
const optimistic = useOptimistic<ReadDoctorDTO>()
const { can } = usePermission()
const canEdit = computed(() => can('doctors', 'edit'))

const formFields = computed<FormField[]>(() => [
  { key: 'first_name', label: t('field.name'), type: 'text', required: true },
  { key: 'last_name', label: t('field.nameFull'), type: 'text' },
  { key: 'patronymic', label: t('field.comment'), type: 'text' }
])

const dialogTitle = computed(() => {
  if (mode.value === 'create') {
    return `${t('button.create')} ${t('resource.doctors')}`
  }
  if (mode.value === 'edit') {
    return `${t('dialog.edit')} ${t('resource.doctors')}`
  }
  return t('resource.doctors')
})
...
const onSave = async (payload: Record<string, any>) => {
  ...
  try {
    if (mode.value === 'create') {
      ...
      const response = await updateDoctor(selected.value.id, payload)
...
const confirmDelete = (row: ReadDoctorDTO) => {
