    ...
    >
      <template #toolbar-actions>
        <Button
          ...
          icon="pi pi-plus"
          v-permission="['research-goals','create']"
          @click="openCreate()"
        />
      </template>
...
import type { ReadResearchGoalDTO } from './research-goals.api'
import {
  createResearchGoal,
  deleteResearchGoal,
  listResearchGoals,
  updateResearchGoal
} from './research-goals.api'
...
const columns = computed<TableColumn[]>(() => [
  ...
  },
  {
    ...
    sortable: true,
    filter: { type: 'text', placeholder: t('filter.default') },
    body: (row: ReadResearchGoalDTO) => row.lab?.name || '-'
  },
  {
    field: 'comment',
  ...
  },
  {
    ...
    body: (row: ReadResearchGoalDTO) => formatDateTime(row.updated_at)
  }
])
...
const table = useServerTable<ReadResearchGoalDTO>(listResearchGoals, {
...
const dialog = useCrudDialog<ReadResearchGoalDTO>('research-goals')
...
const optimistic = useOptimistic<ReadResearchGoalDTO>()
const { can } = usePermission()
const canEdit = computed(() => can('research-goals', 'edit'))
...
const dialogTitle = computed(() => {
  if (mode.value === 'create') {
    return `${t('button.create')} ${t('resource.research-goals')}`
  }
  if (mode.value === 'edit') {
    return `${t('dialog.edit')} ${t('resource.research-goals')}`
  }
  return t('resource.research-goals')
})
...
const onSave = async (payload: Record<string, any>) => {
  ...
  try {
    if (mode.value === 'create') {
      const response = await createResearchGoal(payload)
      table.data.value = [response.data, ...table.data.value]
    } else if (selected.value?.id) {
      const response = await updateResearchGoal(selected.value.id, payload)
