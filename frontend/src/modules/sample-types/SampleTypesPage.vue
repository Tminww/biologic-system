    ...
    >
      <template #toolbar-actions>
        <Button
          ...
          icon="pi pi-plus"
          v-permission="['sample-types','create']"
          @click="openCreate()"
        />
      </template>
...
import type { ReadSampleTypeDTO } from './sample-types.api'
import { createSampleType, deleteSampleType, listSampleTypes, updateSampleType } from './sample-types.api'
...
const columns = computed<TableColumn[]>(() => [
  ...
  },
  {
    ...
    body: (row: ReadSampleTypeDTO) => formatDateTime(row.updated_at)
  }
])
...
const table = useServerTable<ReadSampleTypeDTO>(listSampleTypes, {
...
const dialog = useCrudDialog<ReadSampleTypeDTO>('sample-types')
...
const optimistic = useOptimistic<ReadSampleTypeDTO>()
const { can } = usePermission()
const canEdit = computed(() => can('sample-types', 'edit'))
...
const dialogTitle = computed(() => {
  if (mode.value === 'create') {
    return `${t('button.create')} ${t('resource.sample-types')}`
  }
  if (mode.value === 'edit') {
    return `${t('dialog.edit')} ${t('resource.sample-types')}`
  }
  return t('resource.sample-types')
})
...
const onSave = async (payload: Record<string, any>) => {
  ...
  try {
    if (mode.value === 'create') {
      const response = await createSampleType(payload)
      table.data.value = [response.data, ...table.data.value]
    } else if (selected.value?.id) {
      const response = await updateSampleType(selected.value.id, payload)
...
const confirmDelete = (row: ReadSampleTypeDTO) => {
