...
import type { ReadStatusDTO } from './statuses.api'
import { createStatus, deleteStatus, listStatuses, updateStatus } from './statuses.api'
...
const columns = computed<TableColumn[]>(() => [
  ...
  },
  {
    ...
    body: (row: ReadStatusDTO) => formatDateTime(row.updated_at)
  }
])
...
const table = useServerTable<ReadStatusDTO>(listStatuses, {
  presetKey: 'statuses',
...
const dialog = useCrudDialog<ReadStatusDTO>('statuses')
...
const optimistic = useOptimistic<ReadStatusDTO>()
const { can } = usePermission()
const canEdit = computed(() => can('statuses', 'edit'))
...
const dialogTitle = computed(() => {
  if (mode.value === 'create') {
    return `${t('button.create')} ${t('resource.statuses')}`
  }
  if (mode.value === 'edit') {
    return `${t('dialog.edit')} ${t('resource.statuses')}`
  }
  return t('resource.statuses')
})
...
const onSave = async (payload: Record<string, any>) => {
  ...
  try {
    if (mode.value === 'create') {
      const response = await createStatus(payload)
      table.data.value = [response.data, ...table.data.value]
    } else if (selected.value?.id) {
      const response = await updateStatus(selected.value.id, payload)
...
const confirmDelete = (row: ReadStatusDTO) => {
